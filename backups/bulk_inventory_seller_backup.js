...
(function() {
    ...
    // Configuration
    const CONFIG = {
        DELAY_BETWEEN_SALES: 1000, // Delay between each sale in milliseconds
        MAX_RETRIES: 3,
        RETRY_DELAY: 2000,
        COLORS: {
            primary: '#2196F3',
    ...
    function closeBulkSeller() {
        const modal = document.getElementById('bulk-seller-modal');
        if (modal) {
            modal.remove();
        }
    ...
    async function processSales(mode) {
        ...
        try {
            ...
            for (let i = 0; i < itemsToProcess.length; i++) {
                ...
                try {
                    const result = await processSingleItem(item, mode);
                    if (result.success) {
                        successCount++;
                        const msg = mode === 'bid' ?
